...
const Btn = ({ children, onClick, variant = "primary", disabled, style, type = "button" }) => {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled}
      style={{ padding: "9px 20px", borderRadius: 8, fontSize: 13.5, fontWeight: 600,
...
const Badge = ({ children, color = T.accent }) => (
  <span style={{ display: "inline-block", fontSize: 11, fontWeight: 600, padding: "2px 9px",
    borderRadius: 99, background: color + "22", color, letterSpacing: ".02em" }}>{children}</span>
);
...
const StatusMsg = ({ status }) => status ? (
  <div style={{ marginTop: 14, padding: "11px 15px", borderRadius: 8, fontSize: 13,
...
const Avatar = ({ name, size = 38, bg = "#1e1e40" }) => (
  <div style={{ width: size, height: size, borderRadius: "50%", background: bg,
    ...
    flexShrink: 0, fontFamily: "'Syne', sans-serif" }}>{initials(name)}</div>
);
...
function CostCalculator() {
  ...
  return (
    <Card>
      ...
      <div style={{ background: T.surfaceHi, borderRadius: 10, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {byStage(stage).map(a => (
                <div key={a._id} onClick={() => setSelected(a)}
                  style={{ background: T.surfaceHi, border: `1px solid ${T.border}`, borderRadius: 10, padding: 12, cursor: "pointer" }}
        ...
        {[["schedule", "Schedule Interviews"], ["scheduled", "Scheduled"]].map(([t, l]) => (
          <button key={t} onClick={() => setTab(t)}
            style={{ padding: "9px 22px", border: "none", background: "none", cursor: "pointer",
              ...
              marginBottom: -1, fontFamily: "'Syne',sans-serif", fontSize: 14 }}>{l}
            {t === "scheduled" && interviews.length > 0 && (
              <span style={{ marginLeft: 6, fontSize: 10, padding: "1px 6px", borderRadius: 99, background: T.accent + "33", color: T.accent }}>{interviews.length}</span>
            )}
          </button>
        ))}
      </div>
                ...
                <div key={jobTitle} style={{ marginBottom: 18 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ fontSize: 11, padding: "2px 7px", borderRadius: 99, background: T.accent + "22", color: T.accent }}>{group.length}</span>
                    </div>
                      ...
                      return (
                        <div key={c._id} onClick={() => toggleCandidate(c._id)}
                          style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px",
                            borderRadius: 10, border: `1px solid ${on ? T.accent : T.border}`,
                            background: on ? T.accent + "11" : T.surface, cursor: "pointer", transition: "all .15s" }}>
                          {/* Checkbox */}
        ...
        {[["upload", "Upload Resume"], ["candidates", "All Candidates"]].map(([t, l]) => (
          <button key={t} onClick={() => setTab(t)}
            style={{ padding: "9px 22px", border: "none", background: "none", cursor: "pointer",
              color: tab === t ? T.accent : T.muted, fontWeight: tab === t ? 600 : 400,
              borderBottom: tab === t ? `2px solid ${T.accent}` : "2px solid transparent",
              marginBottom: -1, fontFamily: "'Syne',sans-serif", fontSize: 14 }}>{l}</button>
        ))}
      </div>
              ...
              <div style={{ marginBottom: 14, padding: "12px 16px", borderRadius: 8, background: T.surfaceHi, border: `1px solid ${T.border}` }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {jobs.find(j => j._id === form.jobId).requiredSkills.map(s => (
                    <span key={s} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 99, background: T.accent + "22", color: T.accent }}>{s}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          {candidates.length === 0 ? (
            ...
            <div key={c._id}
              style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 16px",
                borderRadius: 10, background: T.surface, border: `1px solid ${T.border}`, marginBottom: 8 }}>
              {/* Clickable area */}
            ...
            <Card key={r._id}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                {[
                  ...
                  <div key={label} style={{ background: T.surfaceHi, borderRadius: 8, padding: "10px 12px", textAlign: "center" }}>
...
export default function App() {
  ...
  const PAGES = {
  dashboard:     <DashboardPage     key={page} />,
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        <aside style={{ width: collapsed ? 58 : 218, flexShrink: 0, background: T.surface, borderRight: `1px solid ${T.border}`, display: "flex", flexDirection: "column", transition: "width .2s ease", overflow: "hidden" }}>
          <div style={{ padding: "18px 14px 16px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {!collapsed && <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 14, whiteSpace: "nowrap", overflow: "hidden" }}>AI Recruiter</span>}
          </div>
            ...
            {NAV.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => setPage(item.id)}
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 8, border: "none", background: active ? item.color + "22" : "transparent", color: active ? item.color : T.muted, cursor: "pointer", textAlign: "left", transition: "all .15s", whiteSpace: "nowrap", overflow: "hidden" }}>
                  <span style={{ fontSize: 15, flexShrink: 0 }}>{item.icon}</span>
                  {!collapsed && (
                    <>
                      <span style={{ fontSize: 13, fontWeight: active ? 600 : 400, flex: 1 }}>{item.label}</span>
                      <span style={{ fontSize: 9, padding: "2px 5px", borderRadius: 4, background: item.color + "22", color: item.color, fontFamily: "'DM Mono',monospace" }}>{item.member}</span>
                    </>
                  )}
                </button>
              );
          ...
          <button onClick={() => setCollapsed(c => !c)}
            style={{ padding: "12px 0", borderTop: `1px solid ${T.border}`, border: "none", borderTop: `1px solid ${T.border}`, background: "transparent", color: T.muted, cursor: "pointer", fontSize: 13 }}>
            {collapsed ? "→" : "←"}
          </button>
        </aside>
