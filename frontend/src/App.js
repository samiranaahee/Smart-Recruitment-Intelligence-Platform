...
function CostCalculator() {
  ...
  const total =
    fields.ads +
    fields.agency +
    fields.interviews * 2 * fields.hrRate +
    fields.hrHours * fields.hrRate;
  return (
    <Card>
      <p
        style={{
      ...
      >
        {[
          ...
          ["hrHours", "HR Hours Spent"],
          ["hrRate", "HR Hourly Rate ($)"],
        ].map(([k, l]) => (
          <div key={k}>
            <label>{l}</label>
            <input type="number" value={fields[k]} onChange={set(k)} />
          </div>
        ))}
      </div>
      <div
...
function JobsPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setStatus({
        type: "success",
        msg: editId ? "Job updated." : "Job posted!",
      });
      resetForm();
      setShowForm(false);
      setEditId(null);
  ...
  return (
    <div style={{ animation: "fadeUp .4s ease" }}>
      <SectionHeader
        title="Job Postings"
        sub="Manage open roles and skill requirements"
      >
        <Btn
          onClick={() => {
            setShowForm((s) => !s);
        ...
        <Card>
          ...
          >
            ...
            <div style={{ marginLeft: "auto" }}>
              <PipelineTag stage={selected.stage} />
            </div>
          </div>
          ...
          <div style={{ marginTop: 20 }}>
            ...
            >
              {STAGES.map((s) => (
                <Btn
                  key={s}
                  variant={s === selected.stage ? "primary" : "ghost"}
                  onClick={() => {
                    moveStage(selected._id, s);
                    setSelected((p) => ({ ...p, stage: s }));
                  }}
  ...
  return (
    <div style={{ animation: "fadeUp .4s ease" }}>
      <SectionHeader
        ...
        sub="Track candidates through the hiring funnel"
      />
      <div
        style={{
          display: "grid",
          ...
          <div key={stage} style={{ minWidth: 140 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {byStage(stage).map((a) => (
                ...
                >
                  <p style={{ fontWeight: 600, fontSize: 12, marginBottom: 3 }}>
                    {a.candidateName}
                  </p>
...
function InterviewsPage() {
  ...
  const [evalForm, setEvalForm] = useState({
    scores: { technical: 7, communication: 7, problemSolving: 7, cultural: 7 },
    feedback: "",
    recommendation: "Yes",
  });
  ...
  const submitSchedule = async (e) => {
    ...
    if (selected.length === 0)
      return setStatus({
        type: "error",
        msg: "Select at least one candidate.",
      });
    if (!scheduleForm.date || !scheduleForm.time)
      return setStatus({ type: "error", msg: "Date and time are required." });
    ...
    try {
      ...
      setStatus({
        type: "success",
        msg: `Interview scheduled for ${selected.length} candidate(s). Email invites sent!`,
      });
      setSelected([]);
      setScheduleForm({
        date: "",
      ...
      setStatus({
        type: "error",
        msg: err.response?.data?.error || "Failed to schedule interviews.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const submitEval = async (e) => {
    e.preventDefault();
    await axios
      .patch(`${BASE}/interviews/${evalTarget._id}/evaluate`, evalForm)
      .catch(() => {});
    setInterviews((iv) =>
      iv.map((i) =>
        i._id === evalTarget._id
          ? { ...i, status: "Completed", scores: evalForm.scores }
          : i,
      ),
    );
    setEvalTarget(null);
  ...
  return (
    <div style={{ animation: "fadeUp .4s ease" }}>
      <SectionHeader
        ...
        sub="Select candidates, set date & time, send calendar invites"
      />
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              borderBottom:
                tab === t ? `2px solid ${T.accent}` : "2px solid transparent",
              marginBottom: -1,
              fontFamily: "'Syne',sans-serif",
              fontSize: 14,
            }}
      ...
      {tab === "schedule" && (
        ...
        >
          ...
          <div>
            ...
            {candidates.length === 0 ? (
              <Card>
                <p style={{ color: T.muted, textAlign: "center", padding: 20 }}>
                  No candidates yet.
                </p>
              </Card>
            ) : (
              Object.entries(grouped).map(([jobTitle, group]) => {
                const allSelected = group.every((c) =>
                  selected.includes(c._id),
                );
                const someSelected = group.some((c) =>
                  selected.includes(c._id),
                );
                return (
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: allSelected
                            ? T.accent + "22"
                            : "transparent",
                          color: someSelected ? T.accent : T.muted,
                          cursor: "pointer",
                        }}
                      >
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              background: on ? T.accent + "11" : T.surface,
                              cursor: "pointer",
                              transition: "all .15s",
                            }}
                          >
                            ...
                            <div style={{ textAlign: "right" }}>
                              <span
                                style={{
                                  fontWeight: 700,
                                  color: scoreColor(c.skillMatchScore),
                                  fontFamily: "'DM Mono',monospace",
                                  fontSize: 13,
                                }}
                              >
                                {c.skillMatchScore}/100
                              </span>
                            </div>
                          </div>
                        );
              ...
              <form onSubmit={submitSchedule}>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <select
                    ...
                    style={{ background: T.surfaceHi }}
                  >
                    {["Technical", "HR", "Cultural Fit", "Final"].map((o) => (
                      <option key={o}>{o}</option>
                    ))}
                  </select>
                </div>
                ...
                >
                  {loading ? (
                    ...
                    `Schedule for ${selected.length || 0} Candidate(s)`
                  )}
                </Btn>
                <StatusMsg status={status} />
              </form>
              ...
              <form onSubmit={submitEval}>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <select
                    ...
                    style={{ background: T.surfaceHi }}
                  >
                    {["Strong Yes", "Yes", "Maybe", "No", "Strong No"].map(
                      (o) => (
                        <option key={o}>{o}</option>
                      ),
                    )}
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEvalForm((f) => ({ ...f, feedback: e.target.value }))
                    }
                    rows={3}
                  />
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <Btn variant="ghost" onClick={() => setEvalTarget(null)}>
                    Cancel
                  </Btn>
                </div>
              </form>
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <Badge
                          color={i.status === "Completed" ? T.green : T.amber}
                        >
                          {i.status}
                        </Badge>
...
function TalentPage() {
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!resumeFile)
      return setStatus({ type: "error", msg: "Please select a PDF resume" });
    if (!form.jobId)
      return setStatus({ type: "error", msg: "Please select a job position" });
  ...
  return (
    <div style={{ animation: "fadeUp .4s ease" }}>
      <SectionHeader
        title="Talent AI Engine"
        sub="Resume parsing, skill scoring & AI candidate summaries"
      />
      <div
        style={{
          display: "flex",
      ...
      {tab === "upload" && (
        <Card>
          <form onSubmit={handleUpload}>
            ...
            {form.jobId &&
              jobs.find((j) => j._id === form.jobId)?.requiredSkills?.length >
                0 && (
                <div
                  style={{
                    marginBottom: 14,
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, candidateStatement: e.target.value }))
                }
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              />
              {resumeFile && (
                <p style={{ fontSize: 12, color: T.green, marginTop: 6 }}>
                  ✓ {resumeFile.name}
                </p>
              )}
            </div>
      ...
      {tab === "candidates" && !selected && (
        <div>
          ...
          {candidates.length === 0 ? (
            <p style={{ color: T.muted, textAlign: "center", padding: 40 }}>
            ...
            candidates.map((c) => (
              ...
              >
                ...
                <div
                  ...
                  onMouseEnter={(e) =>
                    (e.currentTarget.parentElement.style.borderColor = T.accent)
                  }
                  onMouseLeave={(e) =>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {c.appliedJob?.title && (
                      <p
                        style={{ fontSize: 11, color: T.accent, marginTop: 2 }}
                      >
                        ↳ {c.appliedJob.title}
                      </p>
                    )}
          ...
          >
            ...
            <div>
              ...
              {selected.appliedJob?.title && (
                <p style={{ fontSize: 12, color: T.accent, marginTop: 3 }}>
                  Applied for: {selected.appliedJob.title}
                </p>
              )}
            </div>
          </div>
          <Card style={{ marginBottom: 14 }}>
            ...
            <p
              style={{
                ...
                fontWeight: 800,
                fontFamily: "'Syne',sans-serif",
                color: scoreColor(selected.skillMatchScore),
              }}
            >
              {selected.skillMatchScore}
              <span style={{ fontSize: 16, color: T.muted }}>/100</span>
            </p>
            <div
              style={{
                height: 6,
            ...
            >
              <div
                style={{
                  height: 6,
                  width: `${selected.skillMatchScore}%`,
                  background: scoreColor(selected.skillMatchScore),
                  borderRadius: 99,
                }}
              />
            </div>
              ...
              <Card style={{ marginBottom: 14 }}>
                ...
                <div style={{ overflowX: "auto" }}>
                  ...
                  >
                    ...
                    <tbody>
                      {[...new Set(chosen.flatMap((c) => c.skills || []))]
                        .slice(0, 15)
                        .map((skill) => (
                          <tr key={skill}>
                            <td
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {chosen.map((c, i) => {
                        const score = Math.min(
                          ...
                          Math.round(
                            40 +
                              c.skillMatchScore * 0.4 +
                              (c._id.charCodeAt(0) % 20),
                          ),
                        );
                        return (
...
function RecruitersPage() {
  ...
  const load = () => {
    axios
      .get(`${BASE}/enterprise/recruiters`)
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setStatus({
        type: "error",
        msg: err.response?.data?.error || "Failed to add recruiter.",
      });
    }
  };
  ...
  return (
    <div style={{ animation: "fadeUp .4s ease" }}>
      <SectionHeader
        ...
        sub="Track efficiency, hires, and success rates per recruiter"
      >
      ...
      {showForm && (
        <Card style={{ marginBottom: 20 }}>
          ...
          <form onSubmit={submit}>
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  value={form.email}
                  onChange={set("email")}
                  placeholder="sharmin@company.com"
                  required
                />
              </div>
              <div>
            ...
            >
              <div>
                <label>Hires Made</label>
                <input
                  type="number"
                  min="0"
                  value={form.hiresMade}
                  onChange={set("hiresMade")}
                  placeholder="12"
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  min="0"
                  value={form.avgTimeToHire}
                  onChange={set("avgTimeToHire")}
                  placeholder="14"
                />
              </div>
              <div>
            ...
            <Card key={r._id}>
                ...
                <div style={{ textAlign: "right", marginRight: 16 }}>
                  <p
                    style={{
                      ...
                      fontWeight: 800,
                      fontFamily: "'Syne',sans-serif",
                      color: effColor(r.efficiencyScore),
                    }}
                  >
                    {r.efficiencyScore ?? 0}%
                  </p>
              ...
              >
                <div
                  style={{
                    height: 6,
                    width: `${r.efficiencyScore ?? 0}%`,
                    background: effColor(r.efficiencyScore),
                    borderRadius: 99,
                  }}
                />
              </div>
...
function NotificationsPage() {
  ...
  const send = async (e) => {
    ...
    try {
      ...
      setStatus({
        type: "success",
        msg: "Email queued! (Connect email service in backend)",
      });
    }
  ...
  return (
    <div style={{ animation: "fadeUp .4s ease" }}>
      <SectionHeader
        ...
        sub="Send interview, offer, and rejection emails to candidates"
      />
          ...
          <form onSubmit={send}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <textarea
                ...
                rows={7}
                placeholder="Auto-fills based on type…"
              />
            </div>
            <Btn type="submit" disabled={loading}>
              {loading ? (
...
function DashboardPageContent() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [kpiRes, costRes, trendsRes] = await Promise.all([
          axios.get(`${BASE}/dashboard/kpis`, { headers }).catch(() => null),
          axios
            .get(`${BASE}/dashboard/cost-per-hire`, { headers })
            .catch(() => null),
          axios.get(`${BASE}/dashboard/trends`, { headers }).catch(() => null),
        ]);
        ...
        const costDataDefault = costRes?.data || {
          total_cost: 54600,
          total_hired: 13,
          cost_per_hire: 4200,
        };
        ...
        const trendsData = trendsRes?.data || [
          { month: "1/2026", total_applications: 28, total_hired: 3 },
          { month: "2/2026", total_applications: 35, total_hired: 5 },
          { month: "3/2026", total_applications: 31, total_hired: 4 },
          { month: "4/2026", total_applications: 48, total_hired: 7 },
        ];

        setKpis(kpiData);
        setCostData(costDataDefault);
  ...
  if (loading)
    return (
      <div style={{ color: T.muted, padding: 40 }}>Loading dashboard…</div>
    );

  const funnelStages = kpis
    ? [
        { stage: "Applied", count: kpis.total_applicants, color: T.accent },
        { stage: "Interviewed", count: kpis.total_interviews, color: T.amber },
        { stage: "Offered", count: kpis.total_offers, color: T.green },
        { stage: "Hired", count: kpis.total_hired, color: "#22d3ee" },
      ]
    : [];

  const maxCount = Math.max(...funnelStages.map((f) => f.count), 1);
  const maxTrend = Math.max(...trends.map((t) => t.total_applications || 1), 1);

  return (
    <div style={{ animation: "fadeUp .4s ease" }}>
      <SectionHeader
        title="Recruitment Dashboard"
        sub="Real-time hiring KPIs & funnel analytics"
      />
      <div
        style={{
          display: "grid",
        ...
        {kpis && (
          <>
            <KPICard
              ...
              value={fmt(kpis.total_applicants)}
              accent={T.accent}
            />
            <KPICard
              label="Interview Rate"
              value={kpis.interview_rate}
              accent={T.amber}
            />
            <KPICard
              label="Offer Rate"
              value={kpis.offer_rate}
              accent={T.green}
            />
            <KPICard
              label="Avg Time-to-Hire"
              value={`${kpis.avg_time_to_hire}d`}
              accent="#22d3ee"
            />
            <KPICard
              label="Cost / Hire"
              value={`$${fmt(costData?.cost_per_hire || 0)}`}
              accent={T.red}
            />
            <KPICard
              label="Total Hired"
              value={fmt(kpis.total_hired)}
              accent={T.accentHi}
            />
          </>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {funnelStages.map((f, i) => (
              <div key={f.stage}>
                ...
                <div
                  style={{ height: 7, background: T.border, borderRadius: 99 }}
                >
                  <div
                    style={{
                      height: 7,
            ...
            {trends.map((t) => (
              ...
              >
                ...
                <div
                  style={{
                    width: "100%",
                    height: `${Math.round((t.total_applications / maxTrend) * 100)}%`,
